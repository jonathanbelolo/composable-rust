...
///
/// // Basic usage (without metadata or broadcast)
/// append_events! {
///     store: event_store,
///     stream: "order-123",
...
/// }
///
/// // With broadcast_on_success (for send_and_wait_for detection)
/// // The broadcast_on_success action is broadcast AFTER successful persistence
/// // but does NOT re-enter the reducer - it's broadcast-only.
/// append_events! {
///     store: event_store,
///     stream: "order-123",
///     expected_version: Some(Version::new(5)),
///     events: vec![serialized_event],
///     broadcast_on_success: order_created_event,
///     on_success: |version| Some(OrderAction::VersionUpdated { version }),
///     on_error: |error| Some(OrderAction::AppendFailed { error: error.to_string() })
/// }
/// ```
#[macro_export]
macro_rules! append_events {
    // With metadata and broadcast_on_success
    (
        store: $store:expr,
        stream: $stream:expr,
        ...
        broadcast_on_success: $broadcast:expr,
