...
use ticketing::next::{
    EventEvent, EventProjectionQueries, EventProjector, InventoryEvent, InventoryProjectionQueries,
    InventoryProjector, PaymentEvent, PaymentProjectionQueries, PaymentProjector, ReleaseReason,
};
use ticketing::types::{
...
async fn all_projection_tests() {
    ...
    ep.project(&[ser(
        &EventEvent::Updated {
            ...
            name: None,
            venue: None,
            date: Some(EventDate::new(new_date)),
            updated_at: now,
        },
        "EventUpdated",
    )])
    ...
    .expect("project");
    assert_eq!(
        eq.get_event(eid).await.unwrap().unwrap().date.inner(),
        new_date
    );
    ...
    .expect("project");
    assert_eq!(
        eq.get_event(eid).await.unwrap().unwrap().status,
    ...
    .expect("project");
    assert_eq!(
        eq.get_event(eid2).await.unwrap().unwrap().status,
    ...
    let new_pricing = vec![PricingTier {
        tier_type: TierType::Regular,
        section: "VIP".into(),
        base_price: Money::from_cents(20000),
        available_from: now,
        available_until: None,
    }];
    ep.project(&[ser(
    ...
    ep.project(std::slice::from_ref(&create_ev))
        .await
        .expect("first");
    ep.project(std::slice::from_ref(&create_ev))
        .await
        ...
        .map(|i| {
            ser(
                &EventEvent::Created {
                    ...
                    venue: venue(),
                    date: EventDate::new(now + Duration::days(i.into())),
                    pricing_tiers: pricing(),
                    created_at: now,
                },
                "EventCreated",
    ...
    let ids: Vec<EventId> = eq
        .list_by_owner(pg_owner)
        .await
        .unwrap()
        .iter()
        .map(|e| e.id)
    ...
    let (published, pub_total) = eq
        .list_all(Some(EventStatus::Published), 0, 100)
        .await
        .unwrap();
    ...
    .expect("project");
    let avail = iq
        .get_section_availability(ev_id, "VIP")
        .await
        .unwrap()
        .unwrap();
    ...
    .unwrap();
    let avail = iq
        .get_section_availability(ev_id2, "GA")
        .await
        .unwrap()
        .unwrap();
    ...
    ip.project(&[ser(
        &InventoryEvent::SeatsReserved {
            reservation_id: res_id3,
            event_id: ev_id3,
            section: "Floor".into(),
            seats: seats3[0..3].to_vec(),
            expires_at: now + Duration::minutes(15),
            reserved_at: now,
        },
        "SeatsReserved",
    ...
    .unwrap();
    let avail = iq
        .get_section_availability(ev_id3, "Floor")
        .await
        .unwrap()
        .unwrap();
    ...
    let avail = iq
        .get_section_availability(ev_id4, "Test")
        .await
        .unwrap()
        .unwrap();
    ...
    reset(&pool).await;
    assert!(
        iq.get_section_availability(EventId::new(), "X")
            .await
            .unwrap()
            .is_none()
    );
    ...
    let inv_dto = iq
        .get_inventory_dto(multi_ev, "VIP")
        .await
        .unwrap()
        .unwrap();
    ...
    assert!(
        iq.get_inventory_dto(EventId::new(), "X")
            .await
            .unwrap()
            .is_none()
    );
    ...
    assert!(
        iq.get_reservation_dto(ReservationId::new())
            .await
            .unwrap()
            .is_none()
    );
    ...
    for i in 0..4 {
        pp.project(&[ser(
            &PaymentEvent::PaymentProcessed {
                ...
                amount: Money::from_cents(1000 * (i + 1)),
                payment_method: PaymentMethod::ApplePay,
                processed_at: now,
            },
            "PaymentProcessed",
    ...
    assert!(
        pq.list_by_customer(CustomerId::new())
            .await
            .unwrap()
            .is_empty()
    );
    ...
    ep.project(&[ser(
        &EventEvent::Created {
            event_id: special_id,
            name: "Taylor's \"Eras\" 北京 — 2025!".into(),
            owner_id: UserId::new(),
            venue: venue(),
            date: EventDate::new(now + Duration::days(30)),
            pricing_tiers: pricing(),
    ...
    ip.project(&[ser(
        &InventoryEvent::Initialized {
            event_id: multi_res_ev,
            section: "Main".into(),
            capacity: 100,
            seats: multi_seats.clone(),
            initialized_at: now,
        },
        "InventoryInitialized",
    )])
    ...
    let avail = iq
        .get_section_availability(multi_res_ev, "Main")
        .await
        .unwrap()
        .unwrap();
