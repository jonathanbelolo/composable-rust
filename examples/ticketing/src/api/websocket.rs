...
async fn handle_availability_socket(socket: WebSocket, event_id: Uuid, state: AppState) {
    ...
    // Spawn channel consumer task for real-time updates (using broadcast channels)
    let event_sender = sender.clone();
    let event_inventory_store = state.create_inventory_store(event_id_typed);
    let mut inventory_rx = state.resources.inventory_actions.subscribe();
    let mut reservation_rx = state.resources.reservation_actions.subscribe();

    let mut event_task = tokio::spawn(async move {
        debug!("WebSocket subscribed to inventory and reservation channels for availability updates");

        loop {
            tokio::select! {
                Ok(_action) = inventory_rx.recv() => {
                    // All inventory events affect availability
                ...
                Ok(action) = reservation_rx.recv() => {
                    // Filter reservation events by event_id
                    let matches_event = match &action {
                        crate::aggregates::ReservationAction::ReservationInitiated {
                            event_id: ev_id, ..
...
async fn handle_notifications_socket(socket: WebSocket, session: SessionUser, state: AppState) {
    ...
    // Spawn channel consumer task for personal notifications (using broadcast channels)
    let event_sender = sender.clone();
    ...
    let mut reservation_rx = state.resources.reservation_actions.subscribe();
    let mut payment_rx = state.resources.payment_actions.subscribe();

    let mut event_task = tokio::spawn(async move {
        debug!("WebSocket subscribed to reservation and payment channels for notifications");

        loop {
            tokio::select! {
                Ok(action) = reservation_rx.recv() => {
                    // Filter and transform reservation events by customer_id
                    let notification = match &action {
                        ...
                        crate::aggregates::ReservationAction::ReservationCompleted {
                        ...
                        crate::aggregates::ReservationAction::ReservationExpired {
                            reservation_id, ..
                        } => {
                            // Check ownership via index
                            let belongs_to_user = reservation_ownership
                        ...
                        crate::aggregates::ReservationAction::ReservationCancelled {
                ...
                Ok(action) = payment_rx.recv() => {
                    // Filter and transform payment events by customer_id
                    let notification = match &action {
                        crate::aggregates::PaymentAction::PaymentProcessed {
                        ...
                        crate::aggregates::PaymentAction::PaymentSucceeded {
                        ...
                        crate::aggregates::PaymentAction::PaymentFailed {
                            payment_id, reason, ..
                        } => {
                            // Two-step lookup: payment_id → reservation_id → customer_id
                            let belongs_to_user = payment_ownership
                        ...
                        crate::aggregates::PaymentAction::PaymentRefunded {
